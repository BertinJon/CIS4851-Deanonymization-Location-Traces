...
def hmm_weight(loc_trace, isp_traces):
    ...
    for isp_trace in isp_traces:
        ...
        try:
            ...
            Trans = zeros([Nrg]*2) 
            StsTran_List = [(RGMap(loc_points[t]),RGMap(loc_points[t+1])) for t in range(0,Tmax-1) if t in loc_points and t+1 in loc_points]
...
def main(input_path='./input/', output_path='./output/', save_weights=False):
    ...
    with open(output_path + '/result.txt', 'w') as fw:
        for acc in acc_list:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--region_centre', nargs='?', default='./RegionCenters.txt', help='Path of region centres\' location ')
